            ...
            <div className={`w-[90vw] md:w-[500px] lg:w-[552px] h-[65vh]  md:h-[65vh] lg:h-[68vh] bg-formBg1 bg-no-repeat bg-cover bg-center rounded-[12px] md:rounded-[14px] lg:rounded-[16px] overflow-y-auto gap-y-1.5 md:gap-y-[10px] flex flex-col ${messages.length == 0 ? 'justify-center' : 'justify-start'}  items-center px-[10px] md:px-[13px] lg:px-[15px] pt-[11px] md:pt-[13px] lg:pt-[15px] pb-[45px] md:pb-0`}>
                {messages.length > 0 ?
                    messages.map((el, i) => (
                        <div
                            key={i}
                            className={`w-full  flex justify-start items-start ${user?.uid && user.uid == el.userId ? 'flex-row-reverse' : 'flex-row'} gap-x-[4px] md:gap-x-[5px]`}
                        >
                            <div className="p-[9px] md:p-[10px] lg:p-[11px] bg-formBg rounded-full text-p">
                                <FaRegUser />
                            </div>
                            <div className={`text-p text-[13px] lg:text-[14px] max-w-[50vw] md:max-w-[200px] lg:max-w-[250px] px-[10px] py-[3px] font-semibold bg-formBg rounded-[10px] flex flex-col justify-evenly  ${user?.uid && user.uid == el.userId ? 'rounded-br-[0px] items-end' : 'rounded-bl-[0px] items-start'} `} >
                                <h1 className={`text-[13px] md:text-[14px] lg:text-[15px]  font-bold ${user?.uid && user.uid == el.userId ? 'text-purpleHover' : 'text-purple'}`}>{el.userName}</h1>
